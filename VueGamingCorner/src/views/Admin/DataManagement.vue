<template>
      ...
      <v-window v-model="tab" class="mt-4">
        ...
        <v-window-item value="juegos">
          ...
          >
            ...
            <template #item.actions="{ item }">
              <v-btn icon color="info" @click="editarJuego(item)">
                <v-icon>mdi-pencil</v-icon>
              </v-btn>
              <v-btn icon color="error" @click="eliminarJuego(item)">
        ...
        <v-window-item value="consolas">
          ...
          >
            ...
            <template #item.actions="{ item }">
              <v-btn icon color="info" @click="editarConsola(item)">
                <v-icon>mdi-pencil</v-icon>
              </v-btn>
              <v-btn icon color="error" @click="eliminarConsola(item)">
                <v-icon>mdi-delete</v-icon>
              </v-btn>
            </template>
          </v-data-table>
  ...
  // Datos simulados
  const games = [
    { id: 1, name: 'Elden Ring', platform: 'PC', price: '59.99 €', stock: 10 },
    { id: 2, name: 'God of War', platform: 'PlayStation', price: '49.99 €', stock: 5 },
  ]
  ...
  // Acciones simuladas
  const añadirJuego = () => alert('Añadir nuevo juego')
  const editarJuego = (item: any) => alert('Editar juego: ' + item.name)
  const eliminarJuego = (item: any) => alert('Eliminar juego: ' + item.name)
  
  const añadirConsola = () => alert('Añadir nueva consola')
  const editarConsola = (item: any) => alert('Editar consola: ' + item.model)
